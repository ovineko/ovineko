...
- [x] Проверить что все public API имеют JSDoc комментарии
- [x] Убедиться что generic типы работают корректно для всех React component типов
- [x] Добавить примеры использования в JSDoc
- [x] Проверить что нет `any` типов в коде
- [x] Запустить `pnpm typecheck` (если есть такая команда) или проверить в IDE
...
- [x] Запустить `pnpm lint` - исправить все ошибки
- [x] Запустить полный test suite: `pnpm test`
- [x] Запустить build: `pnpm build` - убедиться что нет ошибок
- [x] Проверить в браузере финальную версию с production build
- [x] Проверить что все файлы отформатированы (prettier/eslint)
