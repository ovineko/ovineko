...
describe("integration: lazyWithRetry error propagation to ErrorBoundary", () => {
  beforeEach(() => {
    disableDefaultRetry();
  });
  ...
  it("error boundary catches chunk error after all lazy retries are exhausted", async () => {
  ...
  it("error boundary catches error when callReloadOnFailure is false", async () => {
...
describe("integration: edge cases - CSP violations and network transitions", () => {
  ...
  it("does not trigger reload for a CSP SecurityError (not a chunk error)", async () => {
