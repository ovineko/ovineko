...
- [x] Create builder.ts with BuildScriptOptions, BuildScriptResult interfaces
- [x] Implement buildSpaGuardScript(): reads pre-built inline scripts from dist-inline/, processes options, generates content hash, builds structured tag objects, generates HTML string
- [x] Implement buildExternalScript(): writes script file to outDir, returns public URL and tags
- [x] Add html-minifier-terser dependency to node package
- [x] Add copy:bundles script to copy dist-inline/ and dist-inline-trace/ from vite package
- [x] Add dist-inline, dist-inline-trace to files array in package.json
- [x] Export buildSpaGuardScript, buildExternalScript from index.ts
- [x] Add `./builder` export path in package.json if needed
- [x] Write tests for buildSpaGuardScript (verify output structure, hash consistency, options processing)
- [x] Write tests for buildExternalScript (verify file creation, public URL generation)
- [x] Run pnpm test in spa-guard/node - must pass
...
- [x] Add mode, externalScriptDir, publicPath to VitePluginOptions interface
- [x] Add @ovineko/spa-guard-node as workspace dependency
- [x] For mode: 'inline' (default) - keep existing behavior unchanged
- [x] For mode: 'external' - use writeBundle hook to call buildExternalScript from node package
- [x] Generate `<script src="...">` tag instead of inline script content for external mode
- [x] Write tests for external mode (verify file generation, correct script tag, hash in filename)
- [x] Verify existing inline mode tests still pass
- [x] Run pnpm test in spa-guard/vite - must pass

### Task 11: Verify acceptance criteria

- [x] All 7 README.md files exist and are under 150 lines
- [x] Root README links point to correct README.md files
- [x] Code examples in READMEs are syntactically correct
- [x] Peer dependencies in READMEs match package.json
- [x] Builder API works standalone (buildSpaGuardScript returns correct structure)
- [x] External mode generates file with content hash
- [x] Inline mode (default) is unchanged
...
- [x] Update spa-guard/node/README.md to mention builder API
- [x] Update spa-guard/vite/README.md to mention external mode option
- [x] Delete TODO-1.md and TODO-2.md
